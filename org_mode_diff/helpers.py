...
def smart_zip(diff_tuple, similarity_function=None):
    ...
    if similarity_function(old_item, new_item):
        return [DiffTuple(old_item, new_item)] + smart_zip(
            DiffTuple(old[1:], new[1:]), similarity_function
        )
    else:
...
def _sequence_similarity_ratio(simplified_old, simplified_new):
    ...
    sequence_match = difflib.SequenceMatcher(None, simplified_old, simplified_new)
