...
from org_mode_diff.diff import (org_items_are_similar, pair_up_subtrees,
                                struct_diff)
from org_mode_diff.models import DiffResult, DiffTuple, OrgHeading, OrgTree


def _make_mock_org_tree(title, todo, tags, text_content, subtrees):
    return OrgTree(
        OrgHeading(1, title, None, todo, tags), (), text_content, subtrees, None, None
    )
...
class TestOrgItemsAreSimilar(unittest.TestCase):
    ...
    def test_different_items(self):
        for different_item in self.different_than_my_item:
            self.assertFalse(org_items_are_similar(self.my_item, different_item))


class TestPairSimilarItems(unittest.TestCase):
    def setUp(self):
        self.item1 = _make_mock_org_tree("testtitle", "TODO", (), "", ())
        self.item2 = _make_mock_org_tree("totally different title", "TODO", (), "", ())
    ...
    def test_unchanged_list(self):
        ...
        self.assertEqual(
            pair_up_subtrees(DiffTuple(first_set_of_items, second_set_of_items)),
            expected,
        )
    ...
    def test_remove_item(self):
        ...
        second_set_of_items = (self.item1,)
    ...
    def test_flip_items(self):
        ...
        result = pair_up_subtrees(DiffTuple(first_set_of_items, second_set_of_items))
...
class TestStructDiff(unittest.TestCase):
    ...
    def test_struct_diff(self):
        diff = struct_diff(DiffTuple(self.old, self.new), False, supress_output=True)
        self.assertEqual(
            diff,
            [
                DiffResult(
    ...
    def test_struct_diff_headers_only(self):
        diff = struct_diff(DiffTuple(self.old, self.new), True, supress_output=True)
        self.assertEqual(
            diff,
            [
                DiffResult(type="comment", prefix="[updated]", string="* Item1"),
